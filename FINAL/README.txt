...
- Ensure that you have placed in the folder a appropriate .obj or .sym file with the same file name 
	- Also ensure that the .sym and .obj files are formatted are correctly
...
- Now that you you have the respective <filename>.lis and <filename>.sic you can open them up to see the results of the XE Dissasembler, (creating these files using your .obj and .sym files)
	- You now can use nano or any other form to open the .sic and .lis
...
- .SYM and .OBJ MUST HAVE CORRECT SPACING or SAME SPACING AS EXAMPLE PROVIDED BY GUY LEONARD
...
- We assume it's a RESW if SIZE mod 3 is 0. Else we assume we RESB
- We assume all Literals are BYTE
- Assuming "0A" does not affect reading in the files  
- We assume that LTORG goes before the final data declarations based off examples from the book vs Leonard's example
...
- Start at addresses other than 0
- We can determine word vs byte by object code
- We can handle multiple literals
- We can handle literals inside code
- Can add the LTORG line or BASE line
- We can jump in memory 

Known deficiencies or bugs:
- If the .sym or .obj is incorrect in contents it will not work as intended
...
So for this project, we met multiple times a week to discuss how we would go about completing this project. From there we would create a minimum viable product that we would have to do which was completed 3-4 weeks
...
- Many comments in the code have been indented to the far right of the page, so scroll right to see all of the comments
