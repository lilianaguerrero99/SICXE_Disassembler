...
//keep this we are using it in calcAddr
...
vector<string> format3(string address, string binary, string label, string hexVal, vector<string> symTable, vector<string> allAddresses, vector<string> literals, vector<string> litTable) {
    ...
    if(getLiteralAtAddress(litTable, address).compare("")!=0){
        operand.append("=X'");
        operand.append(getLiteralAtAddress(litTable, address));
        operand.append("'");
    }
...
masterFormat(vector<vector<string> > total, vector<string> address, vector<string> sortedByFormatSTR,
             vector<string> sortedAndBinarySTR, vector<string> labels, vector<string> symTable,
             vector<string> allAddresses, vector<string> literals, vector<string> litTable) {
        ...
        if (isFormat == true) {
            if (sortedByFormatSTR[i].length() ==
                ...
                total.push_back(format3(address[i], sortedAndBinarySTR[i], labels[i], sortedByFormatSTR[i], symTable,
                                        allAddresses, literals, litTable));
            } else if (sortedByFormatSTR[i].length() ==
