...
int charToInt (basic_string<char> c){ //converts a char to an int
    stringstream ss;
    ss << hex << c;
    int val;
    ss >> val;
...
int stringToInt (string str){ //converts a string into an int
    stringstream ss;
    ss << hex << str;
    int val;
    ss >> val;
...
string intToString (int val){ //converts an int to a string
    stringstream ss;
    ss << hex << val;
    string str = ss.str();
...
vector<vector<string> > appendSymbol(vector<vector<string> > total, vector<string> symTable,  vector<string> allAddresseses, string hRecord) {
    ...
    lastAddress = charToInt(total.at(total.size()-1).at(0));
    for (size_t i=0; i<symTable.size();i++){          // Loop through every line in symbol file and compare address to the last known address
        ...
        int address = stringToInt(toCompare);
        ...
        if(address > lastAddress){  // Append line to total if address is bigger than last address given
            ...
            if(i < symTable.size()-1){  // Make sure not to compare to null string
                int nextAddress = stringToInt(symTable.at(i+1).substr(10,4));
                int size = nextAddress - address;

                string sizeString = intToString(size);
                if(size % 3 == 0){
                    temp.push_back("RESW");
                    size = size / 3;
                ...
                int nextAddress = stringToInt(hRecord.substr(15,4));
                int size = nextAddress - address;
                string sizeString = intToString(size);
                if(size % 3 == 0){
                    temp.push_back("RESW");
...
vector<vector<string> > appendStart(vector<vector<string> > total, vector<string> symTable, string hRecord) {
    ...
    int address = stringToInt(hRecord.substr(9,4));
    toAdd.push_back(intToString(address));
...
vector<vector<string> > appendLTORG(vector<vector<string> > total) {
    ...
    if(lTorg) {  // Append lTorg to line if we set lTorg to true
        ...
        for (vector<string> line : total) {  // Once we add Ltorg exit loop
            int currAdd = charToInt(line.at(0));
            bool done;
...
vector<vector<string> > sortTotal(vector<vector<string>> total){
    ...
    for(size_t i=0; i< total.size(); i++){  //
        int address = charToInt(total.at(i).at(0));
...
void runProgram(string filePath) {
    ...
    total = insertBase(total);  // Appends BASE declarations every time LDB is used
