...
//will convert from binary to hex
...
//will conver from hex to binary
...
//will convert from hex to an int
...
string fillingInt (int value) { //will fill in the missing zeros if needed and make the string uppercase
    stringstream ss;
...
vector<string> format2(string address, string sortedAndBinarySTR, string label, string objectCode) {
    ...
    if (opInst.compare("CLEAR") == 0 || opInst.compare("SVC") == 0 || opInst.compare("TIXR") == 0) {           //  CLEAR, SVC, TIXR
        ...
        int regis2 = hexToInt(reg.getValue(registers(register2)));
    ...
    int reg1 = hexToInt(contentsOfReg1);
    int reg2 = hexToInt(contentsOfReg2);

    // ADDR r1,r2     r2 = (r2) + (r2)
    if(instruction.compare("ADDR") == 0){
        reg2 = reg2 + reg1;
        reg.setValue(registers(register2), fillingInt(reg2));
    }

    //CLEAR r1     r1 <-- 0
    if(instruction.compare("CLEAR") == 0){
        reg1 = 0;
        reg.setValue(registers(register1), fillingInt(reg1));
    }
    ...
    if(instruction.compare("TIXR") == 0){
        int regXI = hexToInt(reg.getValue("X"));

        regXI = regXI + 1;
        reg.setValue("X", fillingInt(regXI));
    }
...
//for addresses, this will add the values of the two addresses and return a 4 bit string
string calcAddr (string one, string two, int index) {
    int value1 = hexToInt(one);
    int value2 = hexToInt(two);
    int total;

    string twosComp = getBinaryStringFromHexString(one);
    char c = twosComp.at(0);
    if (c == '1') { //checks if address is negative to do two's comp
    ...
    if (index == 1) { //checks if the x flag went off so it can add (x) to the disp/addr for the TA
        int value = hexToInt(reg.getValue("X"));
        int newTotal = total + value;
        return fillingInt(newTotal);
    } else {
        return fillingInt(total);
    }
}
...
vector<string> format3(string address, string binary, string label, string hexVal, vector<string> symTable, vector<string> allAddresses, vector<string> literals, vector<string> litTable) {
    ...
    pc = calcAddr(address, "3", 0); //pc is calculated so when the p flag goes off, the TA can be calculated
    ...
    if (binary.substr(6, 2).compare(indr) == 0) {
        ...
        if (binary.at(9) == b) {
            ...
            operand.append(copyOperand);
        } else { //Indirect & Direct
            int intVal = hexToInt(hexVal.substr(3, 3));
        ...
        if (binary.at(10) == p) {
            if (binary.at(8) == '1') { //when X is set to 1
        ...
        } else { //simple and direct
            if (binary.at(8) == '1') { //when X is set to 1
                string intVal = calcAddr(hexVal.substr(3, 3), "0", 1);
                int val = hexToInt(intVal);
                operand.append(to_string(val));
                operand.append(",X");
            } else {
                if ((opcodeTable(binary.substr(0, 6))).compare("RSUB") == 0) {
                    operand.append("");
                } else {
                    int intVal = hexToInt(hexVal.substr(3, 3));
                    operand.append(to_string(intVal));
...
vector<string> format4(string address, string binary, string label, string hexVal, vector<string> symTable) {
    ...
    if (binary.substr(6, 2).compare(immd) ==
        ...
        if (operand.compare("") !=
            ...
            currOp.append(to_string(hexToInt(hexVal.substr(3, 5))));
        }

    } else if (binary.substr(6, 2).compare(indr) ==
...
//goes through every object code that has been split and get the appropriate line base on the length
